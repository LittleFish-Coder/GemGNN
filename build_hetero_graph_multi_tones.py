...
DEFAULT_TONES_SELECTION = 0                          # for tone ablation study (0: all, 1: neutral, 2: affirmative, 3: skeptical, 4: neutral+affirmative, 5: neutral+skeptical, 6: affirmative+skeptical)
# --- Multi-tone Combination Parameters ---
DEFAULT_MULTI_TONE_COUNTS = None                      # for specific tone count combinations (e.g., "2,1,1" for 2 neutral, 1 affirmative, 1 skeptical)
...
class HeteroGraphBuilder:
    ...
    def __init__(
        ...
        tones_selection: int = DEFAULT_TONES_SELECTION,
        multi_tone_counts: str = DEFAULT_MULTI_TONE_COUNTS,
        ...
        self.tones_selection = tones_selection
        self.multi_tone_counts = multi_tone_counts
    ...
    def _get_selected_tones(self):
        """Get the list of tones to include based on tones_selection or multi_tone_counts."""
        # If multi_tone_counts is specified, use the new system
        if self.multi_tone_counts is not None:
            return self._get_multi_tone_selection()
        
        # Otherwise, use the original system
        if self.tones_selection == 0:  # All tones
            return ['neutral', 'affirmative', 'skeptical']
        elif self.tones_selection == 1:  # Neutral only
            return ['neutral']
        elif self.tones_selection == 2:  # Affirmative only
            return ['affirmative']
        elif self.tones_selection == 3:  # Skeptical only
            return ['skeptical']
        elif self.tones_selection == 4:  # Neutral + Affirmative
            return ['neutral', 'affirmative']
        elif self.tones_selection == 5:  # Neutral + Skeptical
            return ['neutral', 'skeptical']
        elif self.tones_selection == 6:  # Affirmative + Skeptical
            return ['affirmative', 'skeptical']
        # Extended combinations for multi-tone support
        elif self.tones_selection == 7:  # 2 Neutral + 1 Affirmative + 1 Skeptical
            return self._get_counted_tones(2, 1, 1)
        elif self.tones_selection == 8:  # 4 Neutral + 2 Affirmative + 2 Skeptical
            return self._get_counted_tones(4, 2, 2)
        elif self.tones_selection == 9:  # 6 Neutral + 3 Affirmative + 3 Skeptical
            return self._get_counted_tones(6, 3, 3)
        elif self.tones_selection == 10:  # 1 Neutral + 2 Affirmative + 1 Skeptical
    ...
    def _get_multi_tone_selection(self):
        """Parse multi_tone_counts string and return appropriate tone selection."""
        try:
            counts = [int(x.strip()) for x in self.multi_tone_counts.split(',')]
            if len(counts) != 3:
                raise ValueError("multi_tone_counts must have exactly 3 values: neutral,affirmative,skeptical")
            neutral_count, affirmative_count, skeptical_count = counts
            return self._get_counted_tones(neutral_count, affirmative_count, skeptical_count)
        except Exception as e:
            raise ValueError(f"Invalid multi_tone_counts format '{self.multi_tone_counts}': {e}")

    def _get_counted_tones(self, neutral_count, affirmative_count, skeptical_count):
        """Return a special marker indicating tone counts rather than simple inclusion."""
        # This will be handled differently in the filtering method
        return {
            'neutral': neutral_count,
            'affirmative': affirmative_count, 
            'skeptical': skeptical_count
        }

    def _get_tones_suffix(self):
        """Get the suffix for naming based on tones_selection or multi_tone_counts."""
        # If multi_tone_counts is specified, use the new system
        if self.multi_tone_counts is not None:
            counts = [int(x.strip()) for x in self.multi_tone_counts.split(',')]
            neutral_count, affirmative_count, skeptical_count = counts
            return f"_tones_{neutral_count}N_{affirmative_count}A_{skeptical_count}S"
        
        # Otherwise, use the original/extended system
        if self.tones_selection == 0:  # All tones - no suffix
            return ""
        elif self.tones_selection == 1:  # Neutral only
            return "_neutral_only"
        elif self.tones_selection == 2:  # Affirmative only
            return "_affirmative_only"
        elif self.tones_selection == 3:  # Skeptical only
            return "_skeptical_only"
        elif self.tones_selection == 4:  # Neutral + Affirmative
            return "_neutral_affirmative"
        elif self.tones_selection == 5:  # Neutral + Skeptical
            return "_neutral_skeptical"
        elif self.tones_selection == 6:  # Affirmative + Skeptical
            return "_affirmative_skeptical"
        elif self.tones_selection == 7:  # 2 Neutral + 1 Affirmative + 1 Skeptical
            return "_tones_2N_1A_1S"
        elif self.tones_selection == 8:  # 4 Neutral + 2 Affirmative + 2 Skeptical
            return "_tones_4N_2A_2S"
        elif self.tones_selection == 9:  # 6 Neutral + 3 Affirmative + 3 Skeptical
            return "_tones_6N_3A_3S"
        elif self.tones_selection == 10:  # 1 Neutral + 2 Affirmative + 1 Skeptical
    ...
    def _filter_interactions_by_tone(self, embeddings_list, tones_list):
        """Filter interaction embeddings and tones based on selected tones or tone counts."""
        selected_tones = self._get_selected_tones()
        
        # Check if we're using the new counting system
        if isinstance(selected_tones, dict):
            return self._filter_interactions_by_count(embeddings_list, tones_list, selected_tones)
        
        # Use the original filtering for simple inclusion
        filtered_embeddings = []
        filtered_tones = []
        
        for embedding, tone in zip(embeddings_list, tones_list):
            tone_key = tone.strip().lower().replace(' ', '_')
            if tone_key in selected_tones:
                filtered_embeddings.append(embedding)
                filtered_tones.append(tone)
        
        return filtered_embeddings, filtered_tones

    def _filter_interactions_by_count(self, embeddings_list, tones_list, tone_counts):
        ...
        for embedding, tone in zip(embeddings_list, tones_list):
            ...
            if tone_key in tone_groups:
                tone_groups[tone_key].append((embedding, tone))
        
        # Sample the requested number from each group
        filtered_embeddings = []
        filtered_tones = []
    ...
    def _add_interaction_edges_by_type(self, data, train_labeled_indices, train_unlabeled_indices, test_indices, num_nodes, num_interactions_per_news, num_interaction_nodes):
        ...
        for idx in train_indices_in_batch:
            ...
            # Filter by selected tones
            filtered_embeddings, filtered_tones = self._filter_interactions_by_tone(embeddings_list, tones_list)
            
            if filtered_embeddings:  # Only process if there are interactions after filtering
                node_interactions = np.array(filtered_embeddings)
                all_interaction_embeddings.append(node_interactions)
                for i, tone in enumerate(filtered_tones):
                    tone_key = tone.strip().lower().replace(' ', '_')
        ...
        # Handle case where no interactions remain after filtering
        if all_interaction_embeddings:
    ...
    def _add_interaction_edges_with_attr(self, data, train_labeled_indices, train_unlabeled_indices, test_indices, num_nodes, num_interactions_per_news, num_interaction_nodes):
        ...
        if all_interaction_embeddings:
            ...
            # Create edge indices using simple arange - but need to adjust for filtered interactions
            actual_interaction_nodes = data['interaction'].num_nodes
            news_has_interaction_src = []
            interaction_has_interaction_tgt = []
            
            # We need to recreate the mapping based on actual filtered interactions
            interaction_idx = 0
            for news_idx in range(num_nodes):
                # Calculate how many interactions this news node actually has after filtering
                if news_idx < len(train_indices_in_batch):
                    # Train node
                    idx = train_indices_in_batch[news_idx]
                    embeddings_list = train_interactions_batch[np.where(train_indices_in_batch == idx)[0][0]]
                    tones_list = train_tones_batch[np.where(train_indices_in_batch == idx)[0][0]]
                else:
                    # Test node
                    test_news_idx = news_idx - len(train_indices_in_batch)
                    idx = test_indices[test_news_idx]
                    embeddings_list = test_interactions_batch[np.where(test_indices == idx)[0][0]]
                    tones_list = test_tones_batch[np.where(test_indices == idx)[0][0]]
                
                filtered_embeddings, _ = self._filter_interactions_by_tone(embeddings_list, tones_list)
                num_filtered = len(filtered_embeddings)
                
                for _ in range(num_filtered):
                    news_has_interaction_src.append(news_idx)
                    interaction_has_interaction_tgt.append(interaction_idx)
                    interaction_idx += 1
            
            if news_has_interaction_src:
                data['news', 'has_interaction', 'interaction'].edge_index = torch.stack([
                    torch.tensor(news_has_interaction_src, dtype=torch.long), 
                    torch.tensor(interaction_has_interaction_tgt, dtype=torch.long)
                ], dim=0)
                data['interaction', 'rev_has_interaction', 'news'].edge_index = torch.stack([
                    torch.tensor(interaction_has_interaction_tgt, dtype=torch.long),
                    torch.tensor(news_has_interaction_src, dtype=torch.long)
                ], dim=0)
            ...
            data['news', 'has_interaction', 'interaction'].edge_index = torch.empty((2, 0), dtype=torch.long)
            data['interaction', 'rev_has_interaction', 'news'].edge_index = torch.empty((2, 0), dtype=torch.long)
            data['news', 'has_interaction', 'interaction'].edge_attr = torch.empty((0,), dtype=torch.long)
            data['interaction', 'rev_has_interaction', 'news'].edge_attr = torch.empty((0,), dtype=torch.long)
    ...
    def save_graph(self, hetero_graph: HeteroData, batch_id=None) -> Optional[str]:
        ...
        tones_suffix = self._get_tones_suffix()
        graph_name = f"{self.k_shot}_shot_{self.embedding_type}_hetero_{self.edge_policy}_{self.k_neighbors}_{sampling_suffix}{tones_suffix}"
...
def main() -> None:
    ...
    if args.multi_tone_counts:
        ...
        print(f"Tones Selection:  {args.tones_selection} ({tones_map.get(args.tones_selection, 'Unknown')})")
    ...
    builder = HeteroGraphBuilder(
        ...
        tones_selection=args.tones_selection,
        multi_tone_counts=args.multi_tone_counts,
