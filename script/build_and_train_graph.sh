        ...
        for embedding_type in "${embedding_types[@]}"; do
            for k_shot in "${k_shots[@]}"; do
                ...
                python train_graph.py \
                    --base_model "$model" \
                    --graph "graphs/${dataset}/${k_shot}shot_${embedding_type}_mutual_knn5.pt" \
                    > "logs/${model}_${dataset}_${k_shot}shot_${embedding_type}_mutual_knn5.log" 2>&1
            done
        done
    done
done
