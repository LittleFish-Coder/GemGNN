...
MASTER_LOG="${LOG_DIR}/build_and_train_hetero_tone_${TIMESTAMP}.log"
        ...
        for EDGE_POLICY in "${EDGE_POLICIES[@]}"; do
            for K_SHOT in "${K_SHOTS[@]}"; do
                for MULTI_VIEW in "${MULTI_VIEWS[@]}"; do
                    for TONE_SELECTION in "${TONES_SELECTIONS[@]}"; do
                        GRAPH_NAME="${K_SHOT}_shot_${EMB}_hetero_${EDGE_POLICY}_${K_NEIGHBORS}_multiview_${MULTI_VIEW}_tone_${TONE_SELECTION}"
                        ...
                        if ! python build_hetero_graph_multi_tones.py \
                            ...
                            --partial_unlabeled \
                            --tones_selection "${TONE_SELECTION}" \
                            --dataset_cache_dir "${DATASET_CACHE_DIR}" \
                            --output_dir "${GRAPH_OUTPUT_BASE_DIR}" > "${BUILD_LOG}" 2>&1; then
                            echo "[ERROR] Failed to build graph for ${DATASET} ${EMB} k-shot:${K_SHOT} edge_policy:${EDGE_POLICY} multiview:${MULTI_VIEW} tone_selection:${TONE_SELECTION}" | tee -a "${MASTER_LOG}"
                            continue
                        fi
