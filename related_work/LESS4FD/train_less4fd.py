...
class SimpleLESS4FDTrainer:
    ...
    def export_test_predictions(self, model_name="less4fd") -> str:
        """Export test predictions in the required CSV format."""
        
        # Extract dataset info from graph path
        # Expected format: graphs_less4fd/less4fd_politifact_k8_deberta.pt
        graph_filename = os.path.basename(self.graph_path).replace('.pt', '')
        parts = graph_filename.split('_')
        
        if len(parts) >= 4:
            dataset_name = parts[1]  # politifact or gossipcop (parts[0] is 'less4fd')
...
def main():
    ...
    trainer.export_test_predictions(model_name="less4fd")
