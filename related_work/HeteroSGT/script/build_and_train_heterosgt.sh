...
if [ ! -f "build_heterosgt_graph.py" ]; then
    print_error "build_heterosgt_graph.py not found. Please run this script from the LESS4FD directory."
    exit 1
fi

if [ ! -f "train_heterosgt.py" ]; then
    print_error "train_heterosgt.py not found. Please run this script from the LESS4FD directory."
    exit 1
fi

# Create output directories
mkdir -p graphs_heterosgt
mkdir -p results_heterosgt
mkdir -p logs
...
EPOCHS=300
PATIENCE=30
RESULTS_DIR="results_heterosgt"

# Setup logging
TIMESTAMP=$(date +%Y-%m-%d_%H-%M-%S)
MASTER_LOG="logs/build_and_train_heterosgt_${TIMESTAMP}.log"

print_status "Starting HeteroSGT experiments at ${TIMESTAMP}" | tee -a "${MASTER_LOG}"
...
for DATASET in "${DATASETS[@]}"; do
    for K_SHOT in "${K_SHOTS[@]}"; do
        ...
        # Build graph command
        if ! python build_heterosgt_graph.py \
