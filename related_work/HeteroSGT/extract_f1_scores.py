...
def extract_f1_scores(results_dir="results_heterosgt"):
    ...
    df = df.sort_values(['dataset', 'k_shot', 'model'])
    ...
    for dataset in df['dataset'].unique():
        ...
        print(f"  Best F1: {best_result['f1']:.4f} (k={best_result['k_shot']}, model={best_result['model']})")
    ...
    # Format for display
    display_df = df[['dataset', 'k_shot', 'model', 'f1', 'accuracy', 'precision', 'recall', 'loss']].copy()
    ...
    for dataset in df['dataset'].unique():
        ...
        for model in dataset_df['model'].unique():
            ...
            print(f"  {model}:")
            for _, row in model_df.sort_values('k_shot').iterrows():
